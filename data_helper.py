...
def pad_X800_same(words, max_len=800):
    words_len = len(words)
    if words_len == max_len:
        return words
    if words_len > max_len:
...
def pad_X400_same(words, max_len=400):
    words_len = len(words)
    if words_len == max_len:
        return words
    if words_len > max_len:
...
def pad_X1000_same(words, max_len=1000):
    words_len = len(words)
    if words_len == max_len:
        return words
    if words_len > max_len:
...
def to_categorical(topics):
    n_sample = len(topics)
    y = np.zeros(shape=(n_sample, 19))

    for i in range(n_sample):
        topic_index = topics[i] - 1
        y[i, topic_index] = 1
    return y
