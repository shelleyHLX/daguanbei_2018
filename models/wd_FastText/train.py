...
# flags.DEFINE_float('last_score12', 0.00, 'if valid_score12 > last_score12, save new model. default: 0.40')
...
def train_epoch(data_path, sess, model, train_fetches,
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        if 0 == (global_step + 1) % 500:
            ...
            batch_id = np.random.randint(0, n_va_batches)  # 随机选一个验证 batch
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        model = network.FastText(W_embedding, settings)
