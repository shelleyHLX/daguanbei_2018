...
class Settings(object):
    def __init__(self):
        self.model_name = 'wd_HCNN_pdQS200'
        self.sent_len = 20
        self.doc_len = 10
        self.sent_filter_sizes = [2, 3, 4, 5]  # [batch_size,  n_filter*len(doc_filter_sizes)]
...
class HCNN(object):
    ...
    def __init__(self, W_embedding, settings):
        ...
        with tf.name_scope('Inputs'):
            print('==================Inputs==================')
            self._X_inputs = tf.placeholder(tf.int64, [None, self.doc_len * self.sent_len], name='X_inputs')
        ...
        with tf.variable_scope('hcnn_content'):
            print('==================hcnn_content==================')
            output = self.hcnn_inference(self._X_inputs)
...
def HCNN_test():
    ...
    with tf.Session(config=config) as sess:
        ...
        loss_list = list()
        for i in range(100):
            X_batch = np.zeros((batch_size, 10 * 30), dtype=float)
