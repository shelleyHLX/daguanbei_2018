...
import os
from data_helper import pad_X1000_same, train_batch

# w
batch_train_path = '../data/wd_pdQS1000/train/'
batch_valid_path = '../data/wd_pdQS1000/valid/'
batch_test_path = '../data/wd_pdQS1000/test/'
...
def train_get_batch(wd_fact2id_path, y_path, batch_path):
    ...
    p = Pool()
    X = np.asarray(list(p.map(pad_X1000_same, facts)), dtype=np.int64)
    p.close()
    p.join()
...
if __name__ == '__main__':
    train_get_batch(file_word2id_va, y_file_valid, batch_valid_path)
